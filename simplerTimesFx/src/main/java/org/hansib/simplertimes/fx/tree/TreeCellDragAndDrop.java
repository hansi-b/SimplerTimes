...
class TreeCellDragAndDrop<T extends TreeItemNode<T>> {
  ...
  private enum DragRelation {
    Below(.7, CSS_CAN_DRAG_TO_BELOW),
    Child(.3, CSS_CAN_DRAG_TO_CHILD),
    Above(.0, CSS_CAN_DRAG_TO_ABOVE);
    ...
    static DragRelation of(DragEvent event, TreeCell<?> cell) {
      ...
      if (yFraction >= Below.threshold) return Below;
      if (yFraction >= Child.threshold) return Child;
      return Above;
    }
  ...
  private record DragTarget<T extends TreeItemNode<T>>(
      TreeItem<T> targetItem, int targetIndex, DragRelation relation) {}
  ...
  private void handleDragDetected(TreeCell<T> cell) {
    ...
    content.putString(
        String.valueOf(cell.getText() != null ? cell.getText() : cell.getTreeItem().getValue()));
  ...
  private DragTarget<T> canDragTo(TreeCell<T> cell, DragEvent event) {
    ...
    if (dragRelation != DragRelation.Child && targetItem.getParent() != null) {
      ...
      targetIndex =
          dragRelation == DragRelation.Above
    ...
    return source.canMoveTo(target, targetIndex)
        ? new DragTarget<>(targetItem, targetIndex, dragRelation)
        : null;
  }

  /**
   * @return either the argument cell's item, if not null; otherwise (for empty cells) the root of
   *     the dragged item
   */
  private TreeItem<T> findTargetItem(TreeCell<T> cell, TreeItem<T> draggedItem) {
    ...
    if (targetItem == null) {
      ...
      while (targetItem.getParent() != null) targetItem = targetItem.getParent();
    }
    return targetItem;
  }
}
