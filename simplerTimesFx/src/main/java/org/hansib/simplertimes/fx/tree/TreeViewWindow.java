...
public class TreeViewWindow<T extends TreeItemNode<T>> {
  ...
  public TreeViewWindow(T root, Runnable itemsChangeHandler) {
    ...
    this.treeView
        .getStylesheets()
        .add(new ResourceLoader().getResourceUrl("fxml/app.css").toString());
  ...
  private TreeView<T> initTreeView(TreeItem<T> rootItem, Runnable changeHandler) {
    ...
    tree.setCellFactory(
        p ->
            dnd.withDragAndDrop(
                new TextFieldTreeCellImpl<T>(changeHandler)
  ...
  private ContextMenu createContextMenu(TextFieldTreeCellImpl<T> cell) {
    ...
    if (treeItem.getParent() == null) return null;
    ...
    return new ContextMenuBuilder()
        ...
        .item(
            ...
            e -> actions.sortChildren(itemsChangeHandler),
    ...
    ContextMenu contextMenu =
        new ContextMenuBuilder()
            ...
            .item(
                ...
                t -> actions.expandChildren(),
                bindings.areAllChildrenExpanded())
            .item(
                MenuItems.CollapseAll.fmt(),
                t -> actions.collapseChildren(),
                bindings.areAllChildrenCollapsed())
            .build();
