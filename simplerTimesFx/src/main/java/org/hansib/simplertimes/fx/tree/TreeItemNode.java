...
public interface TreeItemNode<T extends TreeItemNode<T>> extends TextNode {
  ...
  interface PreRemovalCallback<T> {
    ...
    /**
     * Called when the tree view is asked to remove the argument node. Should check and prepare the
     * imminent removal of the argument node. Can return false to cancel the removal.
     *
     * <p>NB: The implementation must not call {@link TreeItemNode#remove()} on the argument - this
     * is done by the tree.
     *
     * @param node the node submitted for removal
     * @return true if the removal of the argument node from the tree can proceed; if false, the
     *     node is not removed
     */
   ...
   * @param newIndex the target index in the new parent's child list; if this item is already in
   *     that list, then the index is interpreted as the index in the list with the current item
   *     first removed from the list
   */
