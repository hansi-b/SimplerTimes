...
public class Project {
  ...
  public static final Comparator<Project> nameComparator =
      (p1, p2) -> {
  ...
  /**
   * A builder for a project tree. Basically a mutable mirror of Project to allow for predefined
   * ids. Building checks id consistency.
   */
  public static class Builder {
    ...
    /**
     * @return a project tree containing the node of this builder and all its children
     */
    ...
    private Project buildRecursively(
        AtomicLong idGenerator, Project parent, Map<Long, Project> knownProjectsById) {

      if (knownProjectsById.containsKey(id))
        throw Errors.illegalArg(
            "Duplicate id %d: New name '%s', old '%s'", id, name, knownProjectsById.get(id).name);
      ...
      children.forEach(
          cBuilder -> cBuilder.buildRecursively(idGenerator, current, knownProjectsById));
      return current;
    }
  }
  ...
  private Project(
      AtomicLong idGenerator, long id, String name, Project parent, List<Project> children) {
  ...
  /**
   * Checks the preconditions for moving this project to the argument parent project at the argument
   * child list index.
   *
   * <ol>
   *   <li>Both projects must be in the same project tree (have the same id generator).
   *   <li>The new parent must not be the same as or a descendant of this project.
   *   <li>If the new parent is the current parent, the argument index must not be this project's
  ...
  public boolean canMoveTo(Project newParent, int newIndex) {
    if (idGenerator != newParent.idGenerator) return false;
    if (newParent == this.parent) return newIndex != parent.children.indexOf(this);
   ...
   * @param index the target index in the new parent's child list; if this item is already in that
   *     list, then the index is interpreted as the index in the list with the current item first
   *     removed from the list
   * @throws IllegalArgumentException if the preconditions defined by {@link #canMoveTo(Project,
   *     int)} are not fullfilled
   */
  ...
  /**
   * Returns the project with the argument id from this project tree, including this project itself.
   * Only looks "downwards" into children (not upwards at parents).
   *
   * @param id the project id to search for
   * @return the project beneath this project (including this) with the argument id; null if there
   *     is no such project
   */
  ...
  /**
   * @param relativeTo an ancestor of this project below with to begin collecting the words; if it
   *     is null or not an ancestor, all names up to the root are returned
   * @return the ordered list of non-null names, beginning beneath the <code>relativeTo</code>
   *     argument down to this project
   */
  public List<String> namesList(Project relativeTo) {
    LinkedList<String> words = new LinkedList<>();
    for (Project current = this;
        current != relativeTo && current.parent != null;
        current = current.parent) {
      if (current.name != null) words.addFirst(current.name);
    }
    return words;
  }
   ...
   *
   * @return the ordered list of non-null names, beginning with the root down to this project
   */
  ...
  public Stream<Project> filter(Set<String> words) {
    if (words.isEmpty()) return dfStream();
    Set<String> wordsStillMissing =
        name == null
            ? words
    ...
    return wordsStillMissing.isEmpty()
        ? Stream.concat(Stream.of(this), filteredChildren)
        : filteredChildren;
  }
